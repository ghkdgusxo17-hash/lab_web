...
// Get combined events (DB + shared Google Calendar events)
export async function getCombinedEvents(): Promise<{
    dbEvents: Awaited<ReturnType<typeof getEvents>>
    googleEvents: GoogleCalendarEvent[]
    connectedMembers: Awaited<ReturnType<typeof getConnectedMembers>>
}> {
    // Date range: 3 months before ~ 3 months after
    const now = new Date()
    const startDate = new Date(now.getFullYear(), now.getMonth() - 3, 1)
    const endDate = new Date(now.getFullYear(), now.getMonth() + 4, 0)

    const [dbEvents, googleEvents, connectedMembers] = await Promise.all([
        getEvents(),
        getSharedGoogleCalendarEvents(startDate, endDate).catch(err => {
            console.error('[Calendar] Failed to fetch Google events:', err)
            return [] as GoogleCalendarEvent[]
        }),
        getConnectedMembers().catch(() => []),
    ])

    return { dbEvents, googleEvents, connectedMembers }
}
