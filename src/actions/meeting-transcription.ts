...
import { createStorageObjectUrl, removeStoragePaths } from '@/lib/storage-admin'
...
export async function createTranscription(data: { materialId: string; filePath: string; originalFilename: string }) {
    ...
    try {
        const sourceUrl = await createStorageObjectUrl(filePath, 60 * 60)
        ...
        // Start transcription job with a time-limited storage URL.
        startTranscriptionJob(transcription.id, sourceUrl).catch(err => {
