...
import { Package, ShoppingCart, BookOpen, AlertTriangle, Wallet, Network } from 'lucide-react'
import { getInventoryItems, getLowStockItems, getPurchaseRequests, getLedgerSummary } from '@/actions/inventory'
import { getNetworkAssetSummary } from '@/actions/network-asset'
...
export default async function InventoryPage() {
    ...
    // Get summary data
    const [items, lowStockItems, pendingRequests, summary, networkSummary] = await Promise.all([
        getInventoryItems(),
        getLowStockItems(),
                    ...
                    {!isLoggedIn ? (
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span className="text-sm text-slate-500 dark:text-slate-400">IP 관리</span>
                                    </div>
                                    <p className="text-2xl font-bold text-slate-900 dark:text-white">
                                        {networkSummary.total}개
                                    </p>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                ...
                                <Link
                                    href="/inventory/ip"
                                    className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 hover:border-cyan-300 dark:hover:border-cyan-700 transition-all hover:shadow-lg"
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">네트워크 주소 관리</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-slate-500">등록 IP</span>
                                        <span className="font-bold text-cyan-600 dark:text-cyan-400">{networkSummary.total}개</span>
                                    </div>
                                    {networkSummary.unassigned > 0 && (
                                        <div className="mt-2 flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400">
                                            <AlertTriangle className="w-4 h-4" />
                                            <span>{networkSummary.unassigned}개 담당 미지정</span>
                                        </div>
                                    )}
                                </Link>
