...
import { OnlyOfficeViewerClient } from './OnlyOfficeViewerClient'
...
async function getMaterialForViewer(labMeetingId: string, materialId: string) {
...
export async function generateMetadata({
  ...
  const material = await getMaterialForViewer(id, materialId)

  if (!material) {
    return {
      title: '자료 뷰어 | Lab Meeting',
    }
  }

  return {
    title: `${material.title} | 자료 뷰어`,
    description: `${material.labMeeting?.title || 'Lab Meeting'} 자료를 ONLYOFFICE 뷰어로 확인합니다.`,
  }
}

function formatMeetingDate(date: Date | null | undefined) {
  if (!date) {
    return '업로드 자료'
  }
...
export default async function MaterialViewerPage({
  ...
  if (!material || !material.labMeeting) {
    notFound()
  }

  if (!canOpenInOnlyOffice(material)) {
    redirect(`/materials/lab-meeting/${id}`)
  }
              ...
              <div className="min-w-0 space-y-3">
                ...
                <div className="space-y-2">
                  <p className="text-sm font-medium text-cyan-700">
                    {formatMeetingDate(material.labMeeting.date)} · {material.labMeeting.title}
                  </p>
                  <h1 className="text-2xl font-semibold tracking-tight text-slate-900 md:text-3xl">
                    {material.title}
                  </h1>
                  <p className="max-w-3xl text-sm leading-6 text-slate-500 md:text-base">
                    기본 화면은 메뉴를 최소화한 자료 보기 화면입니다.
                    {isSlideMaterial
                      ? ' 발표자료는 슬라이드가 한눈에 잘 보이도록 발표용 보기 기준으로 열립니다.'
                      : ' 문서 내용에만 집중할 수 있도록 뷰어 상단 메뉴를 최대한 줄였습니다.'}
                  </p>
                </div>
              </div>
            ...
            <div className="rounded-[30px] border border-amber-200 bg-white p-8 shadow-[0_20px_70px_rgba(15,23,42,0.08)]">
              <div className="max-w-2xl space-y-3">
                ...
                <h2 className="text-2xl font-semibold text-slate-900">
                  ONLYOFFICE 문서 서버 설정이 아직 비어 있습니다
                </h2>
                <p className="text-sm leading-6 text-slate-600 md:text-base">
                  ONLYOFFICE 환경 변수와 Docker 문서 서버를 준비하면 이 화면에서 바로 자료를 볼 수
                  있습니다.
                </p>
              </div>
            </div>
