...
import { createLabMeeting } from '@/actions/lab-meeting'
...
export function LabMeetingForm({ members }: Props) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        const result = await createLabMeeting(formData)

        if (result.error) {
            alert(result.error)
            setLoading(false)
        } else if (result.success && result.id) {
            router.push(`/materials/lab-meeting/${result.id}`)
        }
    }
