...
function getStatusBadge(user: { isAdmin?: boolean, isApproved?: boolean, role?: string }) {
    if (user.isAdmin) {
        return <span className="text-[10px] font-bold text-white bg-blue-600 px-1.5 py-0.5 rounded-md">ADMIN</span>
    }
    if (user.role === 'ALUMNI') {
        return <span className="text-[10px] font-bold text-white bg-amber-600 px-1.5 py-0.5 rounded-md">ALUMNI</span>
    }
    if (user.isApproved) {
...
export function NavbarClient({ session, unreadInquiryCount = 0, pendingCounts }: NavbarClientProps) {
    ...
    // 비로그인/게스트: 소개 메뉴만, 졸업생: 게시판+캘린더만, 멤버: 모드에 따라 다름
    const getWorkNav = () => isAlumni ? alumniNavigation : workNavigation
    const currentNavigation = !isMember ? introNavigation : (navMode === 'intro' ? introNavigation : getWorkNav())
